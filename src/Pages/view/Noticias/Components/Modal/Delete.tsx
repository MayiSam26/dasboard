...
export default function Delete({ setOpenModalDelete, idNoticia, getNoticias }: props) {
  const [severity, setSeverity] = React.useState<any>("");
  const [mssg, setMssg] = React.useState<any>("");
  const [openAlert, setOpenAlert] = React.useState<boolean>(false);

  const onDelete = () => {
    const url = baseurl + "noticias/delete/" + idNoticia;
    axios
      .delete(url)
      .then((response) => {
        const { data } = response;
    ...
    <>
      ...
      <Box sx={{ px: 3, py: 3.5, textAlign: "center" }}>
        ...
        <Typography variant="body2" sx={{ color: "var(--cya-text-muted)", mt: 0.5 }}>
          ¿Estás segura? Esta acción no se puede deshacer y la quitará también del sitio público si estaba publicada.
        </Typography>
      </Box>
      <Box
        sx={{
